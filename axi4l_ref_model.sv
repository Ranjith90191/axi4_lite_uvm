...
class axi4l_ref_model extends uvm_component;
  ...
  bit [31:0] mem [16]; // Memory implemented in the slave module is limited to MEM_DEPTH (16)[cite: 1]
  ...
  local function void process_write(axi4l_seq_item txn);
    bit [3:0] word_idx = txn.AWADDR[5:2]; // Word address decoding[cite: 1]
    if (txn.AWADDR[1:0] != 2'b00) txn.RESP = `AXI_SLVERR;
    else if (txn.AWADDR > 32'h3F) txn.RESP = `AXI_DECERR;
    else if (word_idx >= 10 && word_idx <= 12) txn.RESP = `AXI_SLVERR; // Read-Only Status registers[cite: 1]
    else begin
      txn.RESP = `AXI_OKAY;
      // Byte Enable Support[cite: 1]
  ...
  local function void process_read(axi4l_seq_item txn);
    ...
    else if (word_idx >= 13 && word_idx <= 14) txn.RESP = `AXI_SLVERR; // Write-Only command registers[cite: 1]
    else begin
      txn.RESP  = `AXI_OKAY;
